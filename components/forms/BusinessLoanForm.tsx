    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input {...register("businessName", { required: true })} placeholder="Your business name" className="input-field" />
        </div>
        <div>
          <label className="label-text">Business Type *</label>
          <select {...register("businessType", { required: true })} className="input-field">
      ...
      <button type="submit" disabled={loading} className="w-full btn-primary justify-center py-4">
        {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Submitting...</> : <><Briefcase className="w-5 h-5" /> Apply for Business Loan</>}
      </button>
    </form>
  );
}
