...
export default function VehicleLoanForm() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (cleanData.vehiclePrice) cleanData.vehiclePrice = parseINR(cleanData.vehiclePrice);
  ...
  if (submitted) {
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-10 px-4">
        ...
        <h3 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-4">Vehicle Loan Application Submitted Successfully</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <input
            ...
            className={"input-field rounded-xl " + (errors.vehicleType ? "border-red-500 focus:ring-red-500" : "focus:border-green-500 focus:ring-green-500/20")} 
          />
        ...
        <div>
          ...
          <input
            ...
            className={"input-field rounded-xl " + (errors.brand ? "border-red-500 focus:ring-red-500" : "focus:border-green-500 focus:ring-green-500/20")} 
          />
          <ErrorMsg error={errors.brand} />
        </div>
        <div>
          ...
          <input
            ...
            className={"input-field rounded-xl " + (errors.model ? "border-red-500 focus:ring-red-500" : "focus:border-green-500 focus:ring-green-500/20")} 
          />
          <ErrorMsg error={errors.model} />
        </div>
        <div>
          <label className="label-text">New or Used Vehicle *</label>
          <select 
            {...register("newOrUsed", { required: "Required" })} 
            className={"input-field rounded-xl " + (errors.newOrUsed ? "border-red-500 focus:ring-red-500" : "focus:border-green-500 focus:ring-green-500/20")}
          >
            <option value="">Select</option>
            <option value="New">New</option><option value="Used">Used</option>
          </select>
          <ErrorMsg error={errors.newOrUsed} />
        </div>
        <div>
          <label className="label-text">Vehicle Price (₹) *</label>
          <Controller
            name="vehiclePrice"
            control={control}
            rules={{ required: "Required" }}
            render={({ field: f }) => (
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="e.g. 5,00,000"
                  className={"input-field pl-9 rounded-xl " + (errors.vehiclePrice ? "border-red-500 focus:ring-red-500" : "focus:border-green-500 focus:ring-green-500/20")}
                />
              </div>
            )}
          />
          <ErrorMsg error={errors.vehiclePrice} />
        </div>
      </div>
