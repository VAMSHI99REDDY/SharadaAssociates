    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input {...register("vehicleModel", { required: true })} placeholder="e.g. Maruti Swift, Honda City" className="input-field" />
        </div>
        <div className="sm:col-span-2">
          <label className="label-text">Required Loan Amount (₹) *</label>
          <input {...register("loanAmount", { required: true })} placeholder="e.g. 800000" className="input-field" />
        </div>
      </div>
      <button type="submit" disabled={loading} className="w-full btn-primary justify-center py-4">
        {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Submitting...</> : <><Car className="w-5 h-5" /> Apply for Vehicle Loan</>}
      </button>
    </form>
  );
}
