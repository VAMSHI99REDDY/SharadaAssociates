...
import { Film, CheckCircle, Loader2 } from "lucide-react";
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await dbInsert("loan_applications", {
        ...
        email: data.email,
        loan_type: "Movie Financing Loan",
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input {...register("productionHouse", { required: true })} placeholder="Production company name" className="input-field" />
        </div>
        <div>
          <label className="label-text">Project Budget (₹) *</label>
          <input {...register("projectBudget", { required: true })} placeholder="Total project budget" className="input-field" />
        </div>
        <div className="sm:col-span-2">
          <label className="label-text">Required Loan Amount (₹) *</label>
          <input {...register("loanAmount", { required: true })} placeholder="e.g. 10000000" className="input-field" />
        </div>
      </div>
      <button type="submit" disabled={loading} className="w-full btn-primary justify-center py-4">
        {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Submitting...</> : <><Film className="w-5 h-5" /> Apply for Movie Financing</>}
      </button>
    </form>
  );
}
